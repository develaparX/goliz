...
- Langsung output teks ini saja:
  "⚠️ **GAMBAR TIDAK JELAS / TERLALU BURAM** 🌫️
...
1. **Pilih Mode Analisa:**
   • /analyst - **Mode Standard** (Swing/Intraday). Mencari setup High Probability & SMC.
   • /scalping - **Mode Scalping** (M1/M5). Sinyal cepat, SL ketat, target pendek.

2. **Kirim Chart:**
   • Kirim **GAMBAR** chart Anda.
   • **WAJIB** tulis nama aset di caption (contoh: "XAUUSD", "BTCUSDT").
   • Jika gambar buram, kirim sebagai **FILE**.

3. **Fitur Tambahan:**
   • **Top-Down Analysis**: Kirim beberapa gambar sekaligus (Album). Bot akan menganalisa dari Timeframe besar ke kecil.
...
**🚀 Mulai sekarang dengan memilih mode di atas!**`
	...
	processAnalysis := func(userID int64, caption string, images [][]byte, chat *tele.Chat) {
		...
		// 3. Status Update
		statusText := "⏳ *Processing Single Chart...*"
		if len(images) > 1 {
			statusText = fmt.Sprintf("⏳ *Processing Top-Down Analysis (%d Charts)...*", len(images))
		}
		statusMsg, err := b.Send(chat, statusText, tele.ModeMarkdown)
		if err != nil {
			log.Printf("❌ [TELEGRAM] Failed to send STATUS to %d: %v", chat.ID, err)
		} else {
			log.Printf("✅ [TELEGRAM] Status sent to %d", chat.ID)
		}
		...
		if err != nil {
			...
			_, errSend := b.Send(chat, "⚠️ Error analyzing (Quota or API Issue). Try again later.")
			if errSend != nil {
				log.Printf("❌ [TELEGRAM] Failed to send ERROR notification to %d: %v", chat.ID, errSend)
			}
			return
		}

		if resp == nil || len(resp.Candidates) == 0 {
			_, errSend := b.Send(chat, "⚠️ No response from AI.")
			if errSend != nil {
				log.Printf("❌ [TELEGRAM] Failed to send NO RESPONSE notification to %d: %v", chat.ID, errSend)
			}
			return
		}
		...
		if err != nil {
			log.Printf("❌ [TELEGRAM] Failed to send FINAL ANALYSIS to %d: %v", chat.ID, err)
		} else {
			log.Printf("✅ [TELEGRAM] Analysis sent to %d (MsgID: %d)", chat.ID, msg.ID)
		}
	}
	...
	b.Handle(&tele.InlineButton{Unique: "disclaimer_btn"}, func(c tele.Context) error {
		return c.Respond(&tele.CallbackResponse{
			Text:      "⚠️ DISCLAIMER: Sinyal ini dihasilkan oleh AI (Artificial Intelligence). Tidak ada jaminan profit 100%. Gunakan money management yang bijak. Do Your Own Research (DYOR).",
			ShowAlert: true,
		})
	})
	...
	b.Handle(tele.OnPhoto, func(c tele.Context) error {
		...
		if mediaGroupID == "" {
			...
			if finalCaption == "" {
				return c.Send("⛔️ Please write the Asset Name in the caption.")
			}
